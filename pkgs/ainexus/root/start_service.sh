...
echo " | |_) | | | | | | | | | \___ \   \ V /  \___ \ ";
echo " |  __/  | |_| | | |_| |  ___) |   | |    ___) |";
echo " |_|      \___/  |____/  |____/    |_|   |____/  (arm64 NVIDIA)";
echo
...
dhcp-match=set:arm-uefi,option:client-arch,6
dhcp-match=set:ipxe,175
dhcp-boot=tag:bios,undionly.kpxe
dhcp-boot=tag:arm-uefi,snponly.efi
...
initrd \${server}/iso/casper/hwe-initrd
kernel \${server}/iso/casper/hwe-vmlinuz initrd=hwe-initrd ip=dhcp url=\${server}workspace/${ISO} autoinstall ds=nocloud-net;s=\${server}user-data/ root=/dev/ram0 cloud-config-url=/dev/null
boot
EOF
)

echo "$dnsmasq_conf_ipxe" > /etc/dnsmasq.conf
echo "$ipxe_ubuntu2204_cfg" > /tftp/ubuntu2204.cfg
